...
const RECRUITMENT_END = '2026-08-28';
const RECRUITMENT_DAYS = 5;
...
function explainSupabaseError(error){
  ...
  if(error?.code==='42501' || lower.includes('permission denied') || lower.includes('row-level security') || lower.includes('row level security')){
    return ['Database permission setup required','Run supabase-upgrade-v6.sql once in Supabase → SQL Editor, then press Retry connection.'];
  }
  if(lower.includes('anonymous') && (lower.includes('disabled') || lower.includes('not enabled'))){
    return ['Anonymous sign-in is disabled','The v6 database supports a public fallback after you run supabase-upgrade-v6.sql. Run the SQL, then retry.'];
  }
...
function subscribeBookings(){
  ...
  realtimeChannel=supabaseClient
    .channel('lso-recruitment-bookings-v6')
...
function renderWeekRange(){
  ...
  $('weekCaption').textContent=open===0?'All 40 interview slots are currently booked.':`${open} of ${VALID_HOURS.length*RECRUITMENT_DAYS} interview slots are currently available.`;
}
...
function renderPanel(){
  ...
  body.innerHTML=`
    ...
    <form class="booking-form" id="bookingForm">
      <div class="field"><label for="applicant">Applicant name <span class="required">*</span></label><input class="control" id="applicant" maxlength="120" required autocomplete="name" placeholder="Full name" /></div>
      ...
      <div class="field"><label for="notes">Notes <span style="color:var(--muted);font-weight:650">(optional)</span></label><textarea class="control" id="notes" maxlength="1000" rows="3" placeholder="Add any recruitment notes"></textarea></div>
...
async function submitBooking(e){
  ...
  if(!isOfficialDate(date)) return showFormError('Please choose a date from August 24 to 28, 2026.');
  ...
  try{
    ...
    else if(error.message==='DATABASE_POLICY'){ showFormError('Database permissions need the v6 SQL upgrade. Please notify the LSO recruitment team.'); }
