...
function configured(){const c=window.LSO_SUPABASE_CONFIG;return !!(c?.url&&c?.publishableKey&&window.supabase?.createClient)}
...
async function startDataLayer(){
  ...
  try{
    ...
    const c=window.LSO_SUPABASE_CONFIG;supabaseClient=window.supabase.createClient(c.url,c.publishableKey,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:false}});
...
function subscribe(){
  ...
  realtimeChannel=supabaseClient.channel('lso-recruitment-v13')
    .on('postgres_changes',{event:'*',schema:'public',table:BOOKINGS_TABLE},()=>scheduleRefresh(false))
    .on('postgres_changes',{event:'*',schema:'public',table:BATCHES_TABLE},()=>scheduleRefresh(true))
    .subscribe(status=>{if(status==='SUBSCRIBED')setStatus('live',currentUid?'Live':'Live · public');else if(status==='CHANNEL_ERROR'||status==='TIMED_OUT'||status==='CLOSED')setStatus('live','Connected · auto refresh')});
}
