...
function explainSupabaseError(error){
  ...
  if(error?.code==='42501' || lower.includes('permission denied') || lower.includes('row-level security') || lower.includes('row level security')){
    return ['Database permission setup required','Run supabase-upgrade-v7.sql once in Supabase → SQL Editor, then press Retry connection.'];
  }
  if(lower.includes('anonymous') && (lower.includes('disabled') || lower.includes('not enabled'))){
    return ['Anonymous sign-in is disabled','The v7 database supports a public fallback after you run supabase-upgrade-v7.sql. Run the SQL, then retry.'];
  }
...
function subscribeBookings(){
  ...
  realtimeChannel=supabaseClient
    .channel('lso-recruitment-bookings-v7')
...
async function submitBooking(e){
  ...
  try{
    ...
    else if(error.message==='DATABASE_POLICY'){ showFormError('Database permissions need the v7 SQL upgrade. Please notify the LSO recruitment team.'); }
