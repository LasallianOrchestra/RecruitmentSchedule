...
function initializeSelection(){
  weekStart=RECRUITMENT_START;
  selected={date:RECRUITMENT_START,hour:START_HOUR};
}
...
function renderWeekRange(){
  ...
  $('weekCaption').textContent=open===0?'This recruitment week is fully booked.':`${open} of ${VALID_HOURS.length*RECRUITMENT_DAYS} interview slots are currently available.`;
}
...
async function submitBooking(e){
  ...
  try{
    ...
    }else if(error.message==='SCHEDULE_RULE_REJECTED'){
      showFormError('This time slot is temporarily unavailable. Please choose another time and notify the LSO recruitment team.');
    }else{
      console.error(error);
