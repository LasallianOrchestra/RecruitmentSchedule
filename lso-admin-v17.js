...
async function ensureClient(){
 ...
 if(!window.LSO_NATIVE_SUPABASE?.createClient)throw new Error('The local secure connection module did not load. Make sure lso-native-supabase-v17.js is uploaded beside admin.html.');
 ...
 *{box-sizing:border-box;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}
 ...
 `}
function buildPrintHtml(dates){const pages=dates.map(buildPrintPage).join('');return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>LSO Interview Schedule</title><base href="${location.href.replace(/[^/]*$/,'')}"><style>${printStyles()}</style></head><body>${pages}</body></html>`}
...
async function doPrint(){
 ...
 const ready=waitForPrintFrame(frame);frame.srcdoc=buildPrintHtml(dates);await ready;await new Promise(r=>setTimeout(r,120));
